...
a_layer1 = sigmoid(h_layer1);

% add bias to a_layer1
a_layer1 = [ones(size(a_layer1, 1), 1), a_layer1];

%% layer 2
h_layer2 = a_layer1*Theta2';
a_layer2 = sigmoid(h_layer2);
...
Theta2(:, 1) = 0;   % remove bias
regularized_term = (lambda/(2.0 * m)) * (sum(sum((Theta1.^2))) + sum(sum((Theta2.^2))));
